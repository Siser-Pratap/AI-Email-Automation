...
import { Plus, X, Building2, Briefcase, Mail, CalendarClock, CheckCircle2, XCircle, Clock, Save, LayoutTemplate, Send, FastForward, ArchiveRestore, Archive, Pencil, User, Hash, RefreshCw, MessageSquarePlus, Trash2, ChevronDown, ChevronUp, ShieldCheck, ShieldX } from "lucide-react";
...
export default function Dashboard() {
  ...
  const reviewCount = emails?.filter((e: any) => e.reviewStatus === "PENDING_REVIEW").length ?? 0;
  const filteredEmails = activeTab === "REVIEW"
    ? emails?.filter((e: any) => e.reviewStatus === "PENDING_REVIEW")
    : emails;
    ...
    <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in duration-500">
      ...
      {isLoading ? (
        ...
        <div className="bg-red-50 text-red-600 p-4 rounded-xl border border-red-200">
          Failed to load emails: {(error as Error).message}. Please check your database connection.
        </div>
      ) : (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm whitespace-nowrap">
              ...
              <tbody className="divide-y divide-gray-100">
                {filteredEmails?.map((entry: any) => (
                  <>
                    <tr key={entry.id} className={cn("hover:bg-gray-50/50 transition-colors", entry.reviewStatus === "PENDING_REVIEW" && "bg-amber-50/40")}>
                            ...
                            {entry.reviewStatus === "PENDING_REVIEW" ? (
                              <>
                                ...
                                <button
                                  ...
                                  title="Approve — add to send queue"
                                >
                                    ...
                                    toast.custom((t) => (
                                      <div className="bg-white rounded-lg shadow-lg p-4 border border-gray-200 flex gap-3">
                                        ...
                                        <div className="flex gap-2">
                                          ...
                                          <button onClick={() => { toast.dismiss(t); deleteMutation.mutate(entry.id); }} className="px-3 py-1 rounded text-sm font-medium bg-red-600 text-white hover:bg-red-700">Discard</button>
                                        </div>
                                      </div>
                                    ));
                                  }}
